...
describe('Students API', () => {
  ...
  describe('POST /api/v1/admin/students', () => {
    ...
    test('should allow creating a student without parent email and skip parent account creation', async () => {
      const token = createToken('admin', 'admin@edu.com', 'HQ');
      const studentId = `s_no_parent_${Date.now()}`;

      const newStudent = {
        id: studentId,
        name: 'Student Without Parent Email',
        classIds: [],
        parentName: 'Parent Optional',
        attendance: 90,
        atRisk: false,
      };
      ...
      expect(created.parentEmail).toBeNull();
      expect(created.parentId).toBeNull();

      // Login should fail because no parent account is created
      const loginRequest = new Request('http://localhost/api/v1/auth/login', {
        method: 'POST',
        body: JSON.stringify({
          email: `autogenerated-${studentId}@edu.com`,
          password: '123',
        }),
      });
      const loginResponse = await worker.fetch(loginRequest, mockEnv, mockCtx);
